...
​Ми можемо анотувати функцію, щоб вказати її тип, що повертається, і типи її параметрів.
"""
...
"""
Це інформує засіб перевірки типів, що у нас є функція mul, яка прийме data як аргумент і поверне float

Викликаємо нашу функцію з неправильним параметром data
"""

# my_mul(1)

"""
Наприклад, у PyCharm IDE це призведе до попередження, що типи параметра, який передається і який очікується не 
збігаються Expected type 'list', got 'int' instead

Це спрацьовує вбудована перевірка типів всередині PyCharm IDE.

А виклик функції зі списком буде коректним, тому що повністю збігається з описом.
"""
print(my_mul([1, 2, 3]))

"""
Одним із найбільш широко використовуваних засобів для перевірки типів є пакет mypy. Якщо ви не використовуєте, 
наприклад, VSCode або PyCharm, або вам потрібно перевірити типізацію в консолі.

Пакет може бути встановлений наступним чином:

            pip install mypy

Після цього можна запустити будь-який файл Python, щоб перевірити, чи збігаються типи.

            mypy part1.py

Після відлагодження помилок можна запускати програму у звичайному режимі.


                        Псевдоніми типів

​Оскільки такі об'єкти, як списки, словники та кортежі містять інші об'єкти, іноді нам потрібно ввести підказки, щоб 
зазначити, які типи об'єктів вони містять. Для цього нам потрібно звернутись до модуля typing, який надає інструменти 
для опису типів."""
...
print(my_mul([1, 4.1, 3]))

"""
У наведеному вище фрагменті знаходиться псевдонім Data, що розшифровується як список значень цілих значень int або з 
плаваючою комою float.

Можна описати типи словника, надавши їх у вигляді списку:
"""
...
​У цьому розділі ми дізналися про модуль типізації Python, який дуже корисний у контексті перевірки типів, 
дозволяючи зовнішнім перевіркам типів точно повідомляти про будь-які помилки. Для детальнішої інформації зверніться 
до документації
"""